...
<div id="gallery" class="bg-white rounded-xl shadow-sm p-6 md:p-8">
    ...
    <!-- Swiper Gallery -->
    <div class="swiper guideGallerySwiper mb-4">
        <div class="swiper-wrapper">
            ...
            @foreach($galleryImages as $index => $image)
                <div class="swiper-slide">
                    <img src="{{ $image }}" alt="Tour Photo {{ $index + 1 }}" class="w-full h-96 object-cover rounded-lg cursor-pointer" onclick="openGuideLightbox({{ $index }})">
                </div>
            @endforeach
        </div>
    ...
    <div class="grid grid-cols-6 gap-2">
        @foreach(array_slice($galleryImages, 0, 6) as $index => $image)
            <img src="{{ $image }}" alt="Thumbnail {{ $index + 1 }}" class="w-full h-20 object-cover rounded cursor-pointer hover:opacity-75 transition-opacity" onclick="openGuideLightbox({{ $index }})">
        @endforeach
    </div>
</div>
...
<script>
    // Initialize Swiper
    const guideSwiper = new Swiper('.guideGallerySwiper', {
        slidesPerView: 1,
        spaceBetween: 10,
        loop: true,
        pagination: {
    ...
    function openGuideLightbox(index) {
