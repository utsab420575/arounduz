...
@section('content')
    ...
    <section class="bg-gradient-to-r from-skyblue to-blue-500 text-white py-8 md:py-12">
        <div class="container mx-auto px-4">
            <div class="max-w-3xl mx-auto text-center">
                ...
                <p class="text-base md:text-lg lg:text-xl opacity-90">Connect with experienced guides who know
                    Uzbekistan inside out</p>
            </div>
        </div>
    </section>
                ...
                <aside class="hidden lg:block lg:w-64 space-y-6">
                    <div class="bg-white rounded-lg shadow-sm p-6 sticky top-24">
                        ...
                        <form action="{{ route('guides') }}" method="GET" id="filterForm">
                                ...
                                <div class="space-y-2">
                                    <label class="flex items-center">
                                        <input type="checkbox" name="status[]" value="online"
                                               {{ in_array('online', request('status', [])) ? 'checked' : '' }} class="rounded text-skyblue focus:ring-skyblue">
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" name="status[]" value="busy"
                                               {{ in_array('busy', request('status', [])) ? 'checked' : '' }} class="rounded text-skyblue focus:ring-skyblue">
                            ...
                            <!-- Price Range -->
                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-700 mb-3">Price Range (per
                                    hour)</label>
                                <div class="space-y-3">
                                    ...
                                    @for($i = 5; $i >= 3; $i--)
                                        <label class="flex items-center">
                                            <input type="radio" name="rating" value="{{ $i }}"
                                                   {{ request('rating') == $i ? 'checked' : '' }} class="text-skyblue focus:ring-skyblue">
                                    ...
                                    @foreach($languages as $lang)
                                        <label class="flex items-center">
                                            <input type="checkbox" name="languages[]" value="{{ $lang }}"
                                                   {{ in_array($lang, request('languages', [])) ? 'checked' : '' }} class="rounded text-skyblue focus:ring-skyblue">
                                    ...
                                    @foreach($specializations as $spec)
                                        <label class="flex items-center">
                                            <input type="checkbox" name="specializations[]" value="{{ $spec }}"
                                                   {{ in_array($spec, request('specializations', [])) ? 'checked' : '' }} class="rounded text-skyblue focus:ring-skyblue">
                            ...
                            <div class="space-y-2">
                                <button type="submit"
                                        class="w-full bg-skyblue text-white py-2 rounded-lg font-semibold hover:bg-blue-600 transition-colors">
                                    Apply Filters
                                </button>
                                <a href="{{ route('guides') }}"
                ...
                <main class="flex-1">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
                            ...
                            <div class="flex gap-2">
                                <div class="flex-1">
                                    <input type="text" name="search" value="{{ request('search') }}"
                                           ...
                                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-skyblue focus:border-skyblue text-sm">
                                </div>
                                <button type="submit"
                                        class="bg-skyblue hover:bg-blue-600 text-white px-6 py-2.5 rounded-lg font-semibold transition-colors">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mt-1">Showing {{ $guides->count() }}
                                    of {{ $guides->total() }} guides</p>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <select name="sort" onchange="this.form.submit()" form="filterForm"
                                        class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-skyblue">
                        ...
                        @if(request()->hasAny(['search', 'status', 'rating', 'languages', 'specializations', 'price_min', 'price_max']))
                            ...
                            <a href="{{ route('guides', array_diff_key(request()->all(), ['search' => ''])) }}"
                            ...
                            {{ ucfirst($status) }}
                            <a href="{{ route('guides', array_merge(request()->all(), ['status' => array_diff(request('status', []), [$status])])) }}"
                    ...
                    @else
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                            ...
                            <a href="{{ route('guides') }}"
                               class="inline-block bg-skyblue text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-600 transition-colors">
                                Clear Filters
                            </a>
                        </div>
                    @endif
    ...
    <div id="mobileFilterModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden lg:hidden">
        <div class="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-xl transform translate-x-full transition-transform duration-300"
            ...
            <div class="h-full flex flex-col">
                ...
                <div class="p-4 border-t border-gray-200 space-y-2">
                    <button type="submit" form="mobileFilterForm"
                            class="w-full bg-skyblue text-white py-3 rounded-lg font-semibold hover:bg-blue-600 transition-colors">
                        Apply Filters
                    </button>
                    <a href="{{ route('guides') }}"
    ...
    <section class="bg-gradient-to-r from-skyblue to-blue-500 text-white py-16">
        <div class="container mx-auto px-4 text-center">
            ...
            <a href="#"
               class="inline-block bg-white text-skyblue px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition-colors">
                Apply Now
            </a>
        </div>
    </section>
