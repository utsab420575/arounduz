...
<section class="bg-gradient-to-r from-purple-600 to-blue-600 text-white py-8 md:py-12">
    <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto text-center">
            ...
            <p class="text-base md:text-lg lg:text-xl opacity-90">Discover professional agencies offering comprehensive travel packages</p>
        </div>
    </div>
</section>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <form action="{{ route('agents') }}" method="GET" id="filterForm">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center">
                                    <input type="checkbox" name="status[]" value="online" {{ in_array('online', request('status', [])) ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-600">
                                    <span class="ml-2 text-sm text-gray-600">Available Now</span>
                                    <span class="ml-auto w-2 h-2 bg-green-500 rounded-full"></span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="status[]" value="busy" {{ in_array('busy', request('status', [])) ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-600">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="types[]" value="{{ $type }}" {{ in_array($type, request('types', [])) ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-600">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                @for($i = 5; $i >= 3; $i--)
                                <label class="flex items-center">
                                    <input type="radio" name="rating" value="{{ $i }}" {{ request('rating') == $i ? 'checked' : '' }} class="text-purple-600 focus:ring-purple-600">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="specializations[]" value="{{ $spec }}" {{ in_array($spec, request('specializations', [])) ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-600">
                        ...
                        <div class="mb-6">
                            <label class="flex items-center">
                                <input type="checkbox" name="verified" value="1" {{ request('verified') ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-600">
                        ...
                        <!-- Action Buttons -->
                        <div class="space-y-2">
                            <button type="submit" class="w-full bg-purple-600 text-white py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors">
                                Apply Filters
                            </button>
                            <a href="{{ route('agents') }}" class="block w-full text-center bg-gray-100 text-gray-700 py-2 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
                                Clear All
                            </a>
                        </div>
                    </form>
            ...
            <main class="flex-1">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
                    ...
                    <form action="{{ route('agents') }}" method="GET" class="mb-4">
                        <div class="flex gap-2">
                            <div class="flex-1">
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search agencies by name or location..." class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:border-purple-600 text-sm">
                            </div>
                            <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-lg font-semibold transition-colors">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">Showing {{ $agents->count() }} of {{ $agents->total() }} agencies</p>
                        </div>
                        
                        <!-- Sort Options -->
                        <div class="flex items-center gap-3">
                            <label class="text-sm font-medium text-gray-700">Sort by:</label>
                            <select name="sort" onchange="this.form.submit()" form="filterForm" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-600">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        @if(request('search'))
                        <span class="inline-flex items-center gap-1 bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs font-medium">
                            Search: "{{ request('search') }}"
                            <a href="{{ route('agents', array_diff_key(request()->all(), ['search' => ''])) }}" class="hover:text-purple-900">
                        ...
                        <span class="inline-flex items-center gap-1 bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                            Verified Only
                            <a href="{{ route('agents', array_diff_key(request()->all(), ['verified' => ''])) }}" class="hover:text-blue-900">
                                <i class="fa-solid fa-times"></i>
                            </a>
                        </span>
                        @endif
                        ...
                        <span class="inline-flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-medium">
                            {{ ucfirst($status) }}
                            <a href="{{ route('agents', array_merge(request()->all(), ['status' => array_diff(request('status', []), [$status])])) }}" class="hover:text-green-900">
                                <i class="fa-solid fa-times"></i>
                            </a>
                        </span>
                        @endforeach
                        
                        <a href="{{ route('agents') }}" class="inline-flex items-center gap-1 bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-medium hover:bg-red-200">
                ...
                <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                    ...
                    <a href="{{ route('agents') }}" class="inline-block bg-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition-colors">
                        Clear Filters
                    </a>
                </div>
                @endif
    ...
    <div class="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-xl transform translate-x-full transition-transform duration-300" id="mobileFilterPanel">
        <div class="h-full flex flex-col">
            ...
            <div class="p-4 border-t border-gray-200 space-y-2">
                <button type="submit" form="mobileFilterForm" class="w-full bg-purple-600 text-white py-3 rounded-lg font-semibold hover:bg-purple-700 transition-colors">
                    Apply Filters
                </button>
                <a href="{{ route('agents') }}" class="block w-full text-center bg-gray-100 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
                    Clear All
                </a>
            </div>
        </div>
...
<section class="bg-gradient-to-r from-purple-600 to-blue-600 text-white py-16">
    <div class="container mx-auto px-4 text-center">
        ...
        <a href="#" class="inline-block bg-white text-purple-600 px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition-colors">
            Register Now
        </a>
    </div>
</section>
